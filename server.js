...
const uuidv4 = () => randomBytes(4).toString('hex').toUpperCase();
...
// ─── Logging ─────────────────────────────────────────────────────────────────
// Placed BEFORE routes so every request is logged (was incorrectly placed after all routes)
...
function broadcast(orderId, payload) {
  const subs = clients.get(String(orderId));
  if (!subs) return;
...
async function advanceOrder(orderId) {
  try {
    ...
    if (next.key === 'picked_up') order.driver = DELIVERY_DRIVERS[Math.floor(Math.random() * DELIVERY_DRIVERS.length)];
    await order.save();

    broadcast(String(order._id), {
      type: 'STATUS_UPDATE',
      order: { id: order._id, status: order.status, history: order.history, driver: order.driver||null, estimatedDelivery: order.estimated_delivery, restaurant: { name: order.real_restaurant_name, location: { lat: order.restaurant_lat, lng: order.restaurant_lng } }, total: order.total, items: order.items, address: order.address },
    });
...
// Silence favicon 404 — return empty 204 if no favicon file present
...
app.get('/api/restaurants/brands', (req, res) => {
  const famous = [
    ...
    // Some other famous ones at the bottom
...
app.get('/api/restaurants/:id/menu', (req, res) => {
  ...
  // If it's a known brand, use its dedicated menu
  let brandMatch = null;
  const lcName = (req.query.name || '').toLowerCase();
  for (const key of Object.keys(GLOBAL_BRANDS)) {
    if (lcName.includes(key)) { brandMatch = key; break; }
  }
...
app.post('/api/orders', verifyTokenOptional, async (req, res) => {
  ...
  try {
    ...
    res.json({ orderId:order._id, order:{ id:order._id, status:'placed', history, items, total, address, restaurant:{name:realRestaurantName,location:restaurantLocation} } });
...
app.get('/api/orders/:id', async (req, res) => {
  try {
    ...
    res.json({ id:order._id, status:order.status, history:order.history, driver:order.driver||null, estimatedDelivery:order.estimated_delivery, restaurant:{name:order.real_restaurant_name,location:{lat:order.restaurant_lat,lng:order.restaurant_lng}}, total:order.total, address:order.address, items:order.items });
...
wss.on('connection', ws => {
  ...
  ws.on('message', async raw => {
    try {
      ...
      if (msg.type === 'SUBSCRIBE' && msg.orderId) {
        ...
        clients.get(subId).add(ws);
        const order = await Order.findById(subId);
        if (order) {
          ws.send(JSON.stringify({ type:'STATUS_UPDATE', order:{ id:order._id, status:order.status, history:order.history, driver:order.driver||null, estimatedDelivery:order.estimated_delivery, restaurant:{name:order.real_restaurant_name,location:{lat:order.restaurant_lat,lng:order.restaurant_lng}}, total:order.total, items:order.items } }));
        } else {
  ...
  ws.on('close', () => { if (subId && clients.has(subId)) clients.get(subId).delete(ws); });
});
